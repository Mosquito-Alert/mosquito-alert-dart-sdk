import 'package:test/test.dart';
import 'package:mosquito_alert/mosquito_alert.dart';

// tests for MetaCreateIdentificationTaskReviewRequest
void main() {
  final instance = MetaCreateIdentificationTaskReviewRequestBuilder();
  // TODO add properties to the builder and call build()

  group(MetaCreateIdentificationTaskReviewRequest, () {
    // String action (default value: 'overwrite')
    test('to test the property `action`', () async {
      // TODO
    });

    // String publicPhotoUuid
    test('to test the property `publicPhotoUuid`', () async {
      // TODO
    });

    // Indicates if the content is safe for publication.
    // bool isSafe
    test('to test the property `isSafe`', () async {
      // TODO
    });

    // String publicNote
    test('to test the property `publicNote`', () async {
      // TODO
    });

    // AnnotationClassificationRequest result
    test('to test the property `result`', () async {
      // TODO
    });

  });
}
